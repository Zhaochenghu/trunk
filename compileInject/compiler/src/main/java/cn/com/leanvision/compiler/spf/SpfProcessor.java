...
public class SpfProcessor extends AbstractProcessor {
  ...
  public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
    ...
    ImmutableList<TypeElement> types = new ImmutableList.Builder<TypeElement>()
        ...
        .build();
    for (TypeElement type : types) { // 获取所有标记@LvPreferences的类
      processType(type);
    }
    return true;
  }
  ...
  private void processType(TypeElement typeElement) {
    ...
    for (Element item : members) {
      ...
      if (name.startsWith("set")) { // 生成所有set保存方法
        if (isObject) {
    ...
    MethodSpec.Builder constructor = MethodSpec.constructorBuilder()
        .addModifiers(Modifier.PUBLIC) // TODO 这里也太不友好了吧！！！肯定是通过注解配置的来啊 com.sojoline.model.storage.AptPreferencesManager
        .addStatement("mPreferences = $T.getContext().getSharedPreferences($S, 0)", ClassName.get("com.sojoline.model.storage", "AptPreferencesManager"), typeElement.getSimpleName())
