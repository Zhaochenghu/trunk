...
import com.sojoline.charging.event.ShowNavListEvent;
import com.sojoline.charging.views.base.LvBaseAppCompatActivity;
import com.sojoline.charging.views.fragments.OrderListFragment;
import com.sojoline.charging.views.fragments.OrderMapFragment;
import com.sojoline.charging.wigets.MapNavListView;
...
import com.sojoline.presenter.collection.CollectionContract;
import com.sojoline.presenter.collection.CollectionPresenter;
import com.sojoline.presenter.usercase.FavorListCase;
import com.sojoline.presenter.usercase.UnFavorCase;
...
//            refreshSubstations(response.substations);
            ...
            .map(new Func1<List<SubstationBean>, List<SubstationBean>>() {
              ...
              public List<SubstationBean> call(List<SubstationBean> substations) {
                ...
                if (!LvTextUtil.isArrayEmpty(substations)) {
                  DebugLog.log("size:" + substations.size());
  ...
  // 刷新收藏列表显示
  private void refreshSubstations(final List<SubstationBean> list) {
    if (!LvTextUtil.isArrayEmpty(list)) {
      tvNo.setVisibility(View.GONE);
      recyclerView.setVisibility(View.VISIBLE);
    } else {
    ...
    DebugLog.log("收藏列表：%d", list.size());
//  Observable.just(substations)
    LvRepository.getInstance().filterFavorSatation()
